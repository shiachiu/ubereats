...
postiveAns = torch.ones([3200,1],dtype=torch.float)
negativeAns = torch.zeros([3200,1],dtype=torch.float)
postiveComments = []
negativeComments = []

for i in range(POSTIVE_COMMENT_STRAT, POSTIVE_COMMENT_STRAT + 3200):
    vec,ans = waimai10k[str(i)]
    postiveComments.append(vec)
postiveComments = torch.FloatTensor(postiveComments)

for i in range(NEGATIVE_COMMENT_START, NEGATIVE_COMMENT_START + 3200):
    vec,ans = waimai10k[str(i)]
...
測試資料 1K
"""
T_POSTIVE_COMMENT_STRAT = 3200
T_NEGATIVE_COMMENT_START = 7200
t_postiveAns = torch.ones([800,1],dtype=torch.float)
t_negativeAns = torch.zeros([800,1],dtype=torch.float)
t_postiveComments = []
t_negativeComments = []

for i in range(T_POSTIVE_COMMENT_STRAT, T_POSTIVE_COMMENT_STRAT + 800):
    vec,ans = waimai10k[str(i)]
    t_postiveComments.append(vec)
t_postiveComments = torch.FloatTensor(t_postiveComments)

for i in range(T_NEGATIVE_COMMENT_START, T_NEGATIVE_COMMENT_START + 800):
    vec,ans = waimai10k[str(i)]
